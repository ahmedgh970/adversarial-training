...
def data_gen(data, batch_size):
    ...
    for l in range(0, d, batch_size):
        X = data[l:l+batch_size, :, :, :]
        yield X



# Data generator (data_file)
# This data ganerator yield X with two channels(adv_sample, norm_sample) and Y with one channel(norm_sample) ==> for adversarial training task.

def data_generator_2c(data_file, batch_size):
    ...
    while True:
      ...
      for offset in range(0, num_samples):
        ...
        for l in range(0, d, batch_size):
          ...
          Y = np.expand_dims(X_train[l:l+batch_size, :, :, 1], axis=-1)
          yield X, Y
...
def calculate_residual(x, x_rec, x_prior):
    # x_prior threshold for BraTS : 0.52, for MSLUB : 0.84 and 0.57
